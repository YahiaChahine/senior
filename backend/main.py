...
# Send the result to a backend server for storage 
...
PROTECTED_PROCESSES = [
    ...
    , "SearchHost.exe",  'smartscreen.exe', "PhoneExperienceHost.exe", 'ShellHost.exe'
]
...
# JavaScript code for Frida hooking
JS_CODE = """
var apiCalls = %s;
var callSequence = [];
var lastCall = -1;
...
def init_csv(file):
    with open(f'{file}', 'a', newline='') as f:
        writer = csv.writer(f)
        header = ['pid'] + [f't_{i}' for i in range(100)]  # features
...
def classify_behavior(api_sequence, model_path='ensemble_model.pkl'):
    """
    ...
    try:
        ...
        if len(result_sequence) > 0:
            ...
            if prediction is not None:
                ...
                if prediction == 1:
                    send_pid_data(pid, get_proc_info(pid)['name'], 1)
                    subprocess.run(["taskkill", "/F", "/PID", str(pid)], check=True)
                    print("ransomware")
                    print(f"Process {pid} killed successfully.")
                else:
                    send_pid_data(pid, get_proc_info(pid)['name'], 0)
                    print("goodware")
...
def is_not_safe_to_monitor(proc_info):
    try:
        if proc_info['name'] in PROTECTED_PROCESSES:
            return True
...
if __name__ == "__main__":
    try:
        print("Running the monitoring script... Press Ctrl+C to stop.")
        main()
        os._exit(0)
